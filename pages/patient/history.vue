  ...
  <view class="main">
    <div class="big_box" v-for="(item, index) in dataList" :key="index">
      <div class="box_one">
        ...
        <div @click="handleReport(item)" class="" style="color: #516bea">
          查看报告
        </div>
      </div>
    ...
    <!-- 备用方法：如果 window.open 无法打开，可以用 iframe 方式展示 -->
    <!-- 这里插入 iframe 预览区 -->
    <!-- 
	<iframe
      :key="previewUrl"
      ref="iframeRef"
      :src="`${previewUrl}?page=hsn#toolbar=0&navpanes=0&scrollbar=0`"
      frameborder="0"
      target="_blank"
      class="word-iframe h-[100%] w-[100%]"
